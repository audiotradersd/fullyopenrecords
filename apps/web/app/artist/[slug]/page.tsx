...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const context = [genres.slice(0, 2).join(" and "), location].filter(Boolean).join(" from ");
  ...
  return pageMetadata({ title: `${name} — Independent Artist`, description: description.slice(0, 155), path: `/artist/${slug}`, image });
}

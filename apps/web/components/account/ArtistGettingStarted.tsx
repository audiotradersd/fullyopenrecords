...
export default function ArtistGettingStarted() {
  ...
  return <main className="overflow-hidden pb-24">
    <section className="relative isolate min-h-[530px] overflow-hidden border-b border-white/10 bg-[#050a16] md:min-h-[590px]">
      ...
      <Container className="relative z-10"><div className="flex min-h-[530px] max-w-2xl flex-col justify-center py-16 md:min-h-[590px]"><p className="font-meta text-xs uppercase tracking-[0.3em] text-[#9fc7f0]">Welcome to Fully Open</p><h1 className="mt-5 font-display text-5xl leading-[0.98] text-white sm:text-6xl md:text-7xl">{isArtist ? <>Your artist page<br />is ready.</> : <>Build your artist page<br />your way.</>}</h1><p className="mt-6 max-w-xl text-base leading-7 text-[#d3dfef] md:text-lg">Thanks for joining Fully Open Records — a home for real music, independent artists and open minds. Here are a few things you can do to bring your music and story to life.</p><div className="mt-8 flex flex-wrap gap-3"><Link href={actionHref}><Button>{loading ? "Get started" : actionLabel}</Button></Link><Link href="/artists"><Button variant="outline">Explore the site</Button></Link></div></div></Container>
    </section>
    ...
    <section className="bg-[#040b15] py-14"><Container className="max-w-4xl"><div className="text-center"><FileAudio className="mx-auto h-6 w-6 text-[#9fc7f0]" /><p className="mt-4 font-meta text-xs uppercase tracking-[0.28em] text-[#9fc7f0]">How to</p><h2 className="mt-3 font-display text-4xl text-white">Quick answers for your next step.</h2></div><div className="mt-10 space-y-3">{artistOnboardingFeatures.map((feature) => <details key={feature.id} id={`help-${feature.id}`} className="scroll-mt-28 rounded-xl border border-white/10 bg-white/[0.03] p-5"><summary className="cursor-pointer text-lg font-medium text-white">{feature.title}</summary><ol className="mt-4 list-decimal space-y-2 pl-5 text-sm leading-6 text-[#c4d1e2]">{feature.steps.map((step) => <li key={step}>{step}</li>)}</ol><Link href={isArtist ? feature.dashboardHref : "/signup"} className="mt-5 inline-block text-sm font-medium text-[#83c5ff] hover:text-white">{isArtist ? `Open ${feature.title}` : "Create an artist page"} <ChevronRight className="inline h-4 w-4" /></Link></details>)}</div><p className="mt-12 text-center font-meta text-xs uppercase tracking-[0.28em] text-[#69819d]">We&apos;re excited to have you here<br />Real music. Open minds.</p></Container></section>
  </main>;
}

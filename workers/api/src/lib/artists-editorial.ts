...
export async function getArtistsEditorialPayload(env: Env) {
  ...
  const map = (row: typeof heroRows[number]) => ({ slug: row.artistSlug ?? "", name: row.artistName ?? "Artist", genres: (row.genres as string[] | null) ?? [], bio: row.bio ?? "", image: row.customImage || resolveArtistImage(row.artistSlug, row.profileImage, row.heroImage), sampleTrack: { title: row.songTitle ?? "", audioUrl: row.audioUrl ?? "" } });
  const hero = heroRows[0];
  return { hero: hero ? { artist: map(hero), track: { title: hero.songTitle ?? "", audioUrl: hero.audioUrl }, badge: hero.customTitle ?? "", description: hero.customSubtitle ?? "", editorialNote: hero.customDescription ?? "", backgroundImage: hero.customHref || resolveArtistImage(hero.artistSlug, hero.profileImage, hero.heroImage), foregroundImage: hero.customImage || resolveArtistImage(hero.artistSlug, hero.profileImage, hero.heroImage) } : null, artists: gridRows.map(map) };
}
